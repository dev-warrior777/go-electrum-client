package bdb

import (
	"bytes"
	"encoding/hex"
	"os"
	"sync"
	"testing"
	"time"

	"github.com/btcsuite/btcd/wire"
	bolt "go.etcd.io/bbolt"
)

var txdb *TxnsDB

func setupTxdb() error {
	bdb, err := bolt.Open("test.bdb", 0600, nil)
	if err != nil {
		return nil
	}
	err = initDatabaseBuckets(bdb)
	if err != nil {
		return nil
	}
	txdb = &TxnsDB{
		db:   bdb,
		lock: new(sync.RWMutex),
	}
	return nil
}

func teardownTxdb() {
	if txdb == nil {
		return
	}
	txdb.db.Close()
	os.RemoveAll("test.bdb")
}
func TestTxnsPut(t *testing.T) {
	if err := setupTxdb(); err != nil {
		t.Fatal(err)
	}
	defer teardownTxdb()
	tx := wire.NewMsgTx(wire.TxVersion)
	txHex := ""
	raw, _ := hex.DecodeString(txHex)
	r := bytes.NewReader(raw)
	tx.Deserialize(r)
	now := time.Now()

	err := txdb.Put(raw, tx.TxHash().String(), 5, 1, now, false)
	if err != nil {
		t.Error(err)
	}

	ret, err := txdb.Get(tx.TxHash())
	if err != nil {
		t.Error(err)
	}

	if ret.Txid.String() != tx.TxHash().String() {
		t.Error("Txns db put failed on Txid")
	}
	if ret.Value != 5 {
		t.Error("Txns db failed on value")
	}
	if ret.Height != 1 {
		t.Error("Txns db failed on height")
	}
	if !now.Equal(ret.Timestamp) {
		t.Error("Txns db failed to return correct time")
	}
	if ret.WatchOnly {
		t.Error("Txns db failed on watchOnly")
	}
	if !bytes.Equal(ret.Bytes, raw) {
		t.Error("Txns db failed on watchOnly")
	}
}

func TestTxnsGet(t *testing.T) {
	if err := setupTxdb(); err != nil {
		t.Fatal(err)
	}
	defer teardownTxdb()
	tx := wire.NewMsgTx(wire.TxVersion)
	txHex := ""
	raw, _ := hex.DecodeString(txHex)
	r := bytes.NewReader(raw)
	tx.Deserialize(r)
	now := time.Now()

	err := txdb.Put(raw, tx.TxHash().String(), 50, 1, now, false)
	if err != nil {
		t.Error(err)
	}

	txn, err := txdb.Get(tx.TxHash())
	if err != nil {
		t.Error(err)
	}

	if tx.TxHash().String() != txn.Txid.String() {
		t.Error("Txn db get failed on txid")
	}
	if txn.Height != 1 {
		t.Error("Txn db failed on height")
	}
	if txn.Value != 50 {
		t.Error("Txns db failed on value")
	}
	if !now.Equal(txn.Timestamp) {
		t.Error("Txn db failed to return correct time")
	}
	if txn.WatchOnly {
		t.Error("Txns db failed on watchOnly")
	}
	if !bytes.Equal(txn.Bytes, raw) {
		t.Error("Txns db failed on watchOnly")
	}
}

func TestTxnsGetAll(t *testing.T) {
	if err := setupTxdb(); err != nil {
		t.Fatal(err)
	}
	defer teardownTxdb()
	tx := wire.NewMsgTx(wire.TxVersion)
	txHex := ""
	raw, _ := hex.DecodeString(txHex)
	r := bytes.NewReader(raw)
	tx.Deserialize(r)

	err := txdb.Put(raw, tx.TxHash().String(), 1, 5, time.Now(), true)
	if err != nil {
		t.Error(err)
	}
	txns, err := txdb.GetAll(true)
	if err != nil {
		t.Error(err)
	}
	if len(txns) < 1 {
		t.Error("Txns db get all failed")
	}
}

func TestDeleteTxns(t *testing.T) {
	if err := setupTxdb(); err != nil {
		t.Fatal(err)
	}
	defer teardownTxdb()
	tx := wire.NewMsgTx(wire.TxVersion)
	txHex := ""
	raw, _ := hex.DecodeString(txHex)
	r := bytes.NewReader(raw)
	tx.Deserialize(r)

	err := txdb.Put(raw, tx.TxHash().String(), 0, 1, time.Now(), false)
	if err != nil {
		t.Error(err)
	}
	txid := tx.TxHash()
	err = txdb.Delete(txid)
	if err != nil {
		t.Error(err)
	}
	txns, err := txdb.GetAll(true)
	if err != nil {
		t.Error(err)
	}
	for _, txn := range txns {
		if txn.Txid == txid {
			t.Error("Txns db delete failed")
		}
	}
}

func TestTxnsDB_UpdateHeight(t *testing.T) {
	if err := setupTxdb(); err != nil {
		t.Fatal(err)
	}
	defer teardownTxdb()
	tx := wire.NewMsgTx(wire.TxVersion)
	txHex := ""
	raw, _ := hex.DecodeString(txHex)
	r := bytes.NewReader(raw)
	tx.Deserialize(r)

	err := txdb.Put(raw, tx.TxHash().String(), 0, 1, time.Now(), false)
	if err != nil {
		t.Error(err)
	}
	err = txdb.UpdateHeight(tx.TxHash(), -1, time.Now())
	if err != nil {
		t.Error(err)
	}
	txn, err := txdb.Get(tx.TxHash())
	if err != nil {
		t.Error(err)
	}
	if txn.Height != -1 {
		t.Error("Txn db failed to update height")
	}
}
